$(window).on('load', ()=> {
  ...
  if(nowurl.match(/edit/)){
    ...
    $('.label-box').on('change', function(e) {
      ...
      if (num != 9){
        ...
        $('.image-box__images').after(buildFileField(num+1));
  ...
  $('#image-box').on('click', '.image__delete', function() {
...
//     $('.image-box__images__input').on('change', function(e) {
//       const targetIndex = $(this).parent().data('index');
//       const num = $(".image-box__images__box").length;
//       const file = e.target.files[0];
//       const blobUrl = window.URL.createObjectURL(file);
...
//   $('#image-box').on('click', '.image-box__images__box__delete', function() {
//     console.log("bbb")

//     $(this).parent().remove();
//     // 画像入力欄が0個にならないようにしておく
//     if ($('.image-box__images__input').length == 0) $('#image-box').append(buildFileField(fileIndex[0]));
//   });
